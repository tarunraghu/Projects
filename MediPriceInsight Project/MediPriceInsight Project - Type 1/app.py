...
def split_csv_file(file_path, ingestion_strategy='type1'):
    """Split the CSV file into address and data files (streaming, memory-efficient)."""
    try:
        ...
        with open(file_path, 'r', encoding=file_encoding, errors='replace') as infile:
            reader = csv.reader(infile)
            # Read the first three rows only
            header_row = next(reader, None)
            address_row = next(reader, None)
            data_header = next(reader, None)

            if not header_row or not address_row or not data_header:
                raise ValueError("CSV file must have at least 3 rows (header, address, data header)")

            # Check if first column is blank in both header and address
            ...
            clean_data_header = [col.strip() if col else f"_c{i}" for i, col in enumerate(data_header)]

            # Write data file (streaming rows)
            with open(data_file, 'w', newline='', encoding='utf-8') as datafile:
                writer = csv.writer(datafile)
                writer.writerow(clean_data_header)
                for row in reader:
    ...
    try:
        ...
        if ingestion_strategy == 'type2':
            ...
            for old_col, new_col in column_mapping.items():
                # For numeric columns, cast to decimal and handle nulls
                if 'charge' in new_col or 'amount' in new_col:
                    # First, clean the string by removing any non-numeric characters except decimal point and negative sign
                    cleaned_col = regexp_replace(col(old_col), '[^0-9.-]', '')
                    # Cast to decimal, defaulting to null if cast fails
                    select_exprs.append(
                        when(cleaned_col.cast('decimal(38,2)').isNotNull(),
                             cleaned_col.cast('decimal(38,2)'))
                        .otherwise(None)
                        .alias(new_col)
                    )
                else:
            ...
            # Add estimated_amount column as null
            select_exprs.append(lit(None).cast('decimal(38,2)').alias('estimated_amount'))
            ...
            for numeric_col in numeric_columns:
                if numeric_col in df_cpt.columns:
                    df_cpt = df_cpt.withColumn(
                        numeric_col,
                        when(col(numeric_col).isNotNull(),
                             spark_round(col(numeric_col).cast('decimal(38,2)'), 2))
                        .otherwise(None)
                    )
            ...
            # Original Type 1 processing logic with the same numeric handling improvements
            # Log all columns for debugging
            logger.info(f"Available columns: {df.columns}")
